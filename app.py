    ...
    if request.method == 'POST':
        ...
        for c in eligibility:
            if c=="BTech-ME":
                ...
                finelf+='#Bachelor of Technology, Civil Engineering'
            elif c=="BTech-CS":
                ...
                finelf+='#Bachelor of Technology, Information Technology'
...
def login():
    ...
    if request.method == 'POST' and 'username' in request.form and 'password' in request.form:
        ...
        if account:
            ...
            if check_password_hash(password_rs, password) and account['role'] =="user":
...
def loginadmin():
    ...
    if request.method == 'POST' and 'username' in request.form and 'password' in request.form:
        ...
        if account:
            ...
            print(password_rs)
            # If account exists in users table in out database
            if check_password_hash(password_rs, password) and account['role']=="admin":
                # Create session data, we can access this data in other routes
                print("hi")
                session['loggedin'] = True
                #session['id'] = account['id']
...
@app.route('/logindeptadmin/', methods=['GET', 'POST'])
def logindeptadmin():
    cursor = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
    # Check if "username" and "password" POST requests exist (user submitted form)
    if request.method == 'POST' and 'username' in request.form and 'password' in request.form:
        ...
        # Check if account exists using MySQL
        cursor.execute('SELECT * FROM deptadmins WHERE username = %s', (username,))
        # Fetch one record and return result
        account = cursor.fetchone()
        ...
        if account:
            ...
            print(password_rs)
            # If account exists in users table in out database
            if check_password_hash(password_rs, password)  and account['role'] == "deptadmin":
                # Create session data, we can access this data in other routes
                session['loggedin'] = True
                #session['id'] = account['id']
                session['username'] = account['username']
                session['role'] = account['role']
                # Redirect to home page
                return redirect(url_for('deptAdminHome'))
            else:
                # Account doesnt exist or username/password incorrect
                print("hi")
                flash('Incorrect username/password')
    ...
    return render_template('logindeptadmin.html')
...
def register():
    ...
    if request.method == 'POST' and 'username' in request.form and 'password' in request.form and 'email' in request.form:
        ...
        if account:
            ...
            cursor.execute("INSERT INTO users (fullname, username, password, role, email) VALUES (%s,%s,%s,%s,%s)", (fullname, username, _hashed_password,"user", email))
            conn.commit()
...
def registerAdmin():
    ...
    if request.method == 'POST' and 'username' in request.form and 'password' in request.form and 'email' in request.form:
        ...
        if account:
            ...
            # Account doesnt exists and the form data is valid, now insert new account into users table
            cursor.execute("INSERT INTO admins (fullname, username, password, role, email) VALUES (%s,%s,%s,%s, %s)", (fullname, username, _hashed_password, "admin", email))
            conn.commit()
            flash('You have successfully registered!')
    elif request.method == 'POST':
        # Form is empty... (no POST data)
...
@app.route('/registerDeptAdmin', methods=['GET', 'POST'])
def registerDeptAdmin():
    cursor = conn.cursor(cursor_factory=psycopg2.extras.DictCursor)
    ...
    if request.method == 'POST' and 'username' in request.form and 'password' in request.form and 'email' in request.form:
        ...
        if account:
            ...
            # Account doesnt exists and the form data is valid, now insert new account into users table
            cursor.execute("INSERT INTO deptadmins (fullname, username, password, role, email) VALUES (%s,%s,%s,%s,%s)", (fullname, username, _hashed_password,"deptadmin", email))
            conn.commit()
            flash('You have successfully registered!')
    elif request.method == 'POST':
        # Form is empty... (no POST data)
        flash('Please fill out the form!')
    # Show registration form with message (if any)
    return render_template('registerdeptadmin.html')
...
def logoutDeptAdmin():
   ...
   session.pop('role', None)
   # Redirect to login page
   return redirect(url_for('logindeptadmin'))
...
@app.route('/deptAdminHome')
def deptAdminHome():
    return render_template('deptadminhome.html')
...
def view_jobs():
    ...
    if a is not None:
        cur = conn.cursor()
        cur.execute('SELECT job_id FROM stats WHERE regno= %s',(session['username'],))
        job_id = cur.fetchone()
        cur = conn.cursor()
        cur.execute('SELECT * FROM job WHERE job_id= %s',job_id)
        job_name = cur.fetchone()
        return render_template('placed.html',job_name = job_name)
...
def viewstat():
    ...
    low=lowest[0]
    high=highest[0]
    s = "SELECT UG.branch, COUNT(*) FROM stats, UG WHERE stats.regNo = UG.regNo GROUP BY UG.branch;"
    cur.execute(s)
    branch_distribution = cur.fetchall()
    ...
    if avg:
        ...
        ave=0
    return render_template('stats.html', branch_distribution=branch_distribution,list_users = list_users,intern =intern,average=ave,lowest=low,highest=high,bt=bt,mt=mt,m=m,ft=ft,np=np,i=i)
...
def search_student():
    ...
    # Map the 3rd digit of regNo to a branch
    branch_mapping = {
        '5': 'Computer Science Engineering',  # Example mapping
    ...
    if request.method == 'POST':
        ...
        branch_input = request.form.get('branch')  # This will be entered by the user
        
        # Start building the SQL query dynamically based on user input
        query = "SELECT regNo, firstName, lastName FROM student WHERE TRUE"
        params = []
        ...
        if first_name:
            query += " AND firstName ILIKE %s"
            params.append('%' + first_name + '%')
        if last_name:
            query += " AND lastName ILIKE %s"
        ...
        # Filter students by branch (based on regNo 3rd digit)
        if branch_input:
            branch_digit = branch_input[0]  # Get the first character (branch input like "CS", "IT")
            filtered_students = []
            for student in students:
                reg_no = student[0]
                branch = None
                if len(reg_no) >= 3:
                    reg_branch_digit = reg_no[2]  # 3rd digit (index 2)
                    branch = branch_mapping.get(reg_branch_digit, 'Unknown')
                if branch and branch_input.lower() in branch.lower():  # Compare input branch
